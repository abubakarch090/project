import { useRouter } from "next/router";
import { useEffect } from "react";
import AOS from "aos";
import "aos/dist/aos.css";  // Import AOS styles

const LoginModule = () => {
    const router = useRouter();

    useEffect(() => {
        AOS.init({ duration: 1000 }); // Initialize AOS with a duration of 1000ms
    }, []);

    return (
        <>
            <div className='container bg-white mx-auto mt-20 mb-20 w-[90%] lg:w-1/2 h-auto rounded-lg p-6 shadow-lg' data-aos="zoom-in">

                <div className='flex flex-col items-center'>

                    <div className="heading">
                        <h2 className="text-center mb-4 text-2xl font-bold" data-aos="fade-up">Log in to your account</h2>
                    </div>

                    <div className='w-[90%] lg:w-[75%] m-auto'>
                        <div className="input w-full m-auto">
                            <input
                                className="px-4 h-12 my-2 border-2 w-full rounded-lg"
                                type="text"
                                placeholder='Email address'
                                data-aos="fade-up"
                            />
                        </div>

                        <div className="input">
                            <input
                                className="px-4 h-12 my-2 border-2 w-full rounded-lg"
                                type="password"
                                placeholder='Password'
                                data-aos="fade-up"
                            />
                        </div>
                    </div>

                    <button className='bg-red-600 hover:bg-red-700 my-2 p-2 w-[90%] lg:w-3/4 rounded-full text-white font-bold text-lg' data-aos="fade-up">
                        Login
                    </button>

                    <div>
                        <h2 className='my-2' data-aos="zoom-in">Don’t have an account? <span className='text-red-500 hover:underline cursor-pointer'
                            onClick={() => router.push("/Signup")}
                        >Sign up</span></h2>
                    </div>
                </div>
            </div>
        </>
    )
}

export default LoginModule;
